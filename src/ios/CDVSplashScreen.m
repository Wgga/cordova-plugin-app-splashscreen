...
- (void)createViews
{
	...
	IsOpenAnimationValue = [self.commandDelegate.settings objectForKey:[@"IsOpenAnimation" lowercaseString]];
	//20230417 :若存在则创建imageview视图
	if([IsOpenAnimationValue boolValue]){
...
{
	// 定义图像视图名称数组
    NSArray *imageNames = @[@"textbg",@"airplane",@"light"];
	// 定义图像视图数组，方便在隐藏视图函数中调用
    imageViews = [[NSMutableArray alloc] init];
	// 获取父视图容器
    UIView *parentView = self.viewController.view;
	// 设置动画的持续时间
    id splashDurationString = [self.commandDelegate.settings objectForKey: [@"SplashScreenDelay" lowercaseString]];
    animationduration = splashDurationString == nil ? kSplashScreenDurationDefault : [splashDurationString floatValue];
	// 设置视图的大小
    viewSize = [UIApplication sharedApplication].delegate.window.bounds.size;
	// 循环创建图像视图
    for (NSString *imageName in imageNames) {
		...
		// 设置图像
        NSString *imagePath = [NSString stringWithFormat:@"www/assets/img/splashbg/%@.png",imageName];
        UIImage *image = [UIImage imageNamed:imagePath];
		// 将图像视图的框架设置为视图的大小
        imageView.frame = CGRectMake(0, 0, viewSize.width, viewSize.height);
        imageView.image = image;
		// 将图像视图的内容模式设置为缩放纵横比填充
        imageView.contentMode = UIViewContentModeScaleAspectFill;
		// 如果图像存在，则向图像视图添加动画
        if(imageView.image){
			// 如果图像是飞机，则添加平移和缩放动画
            if([imageName isEqualToString:@"airplane"]) {
                imageView.center = CGPointMake(imageView.center.x + 50, imageView.center.y + 30);
                [UIView transitionWithView:imageView duration:(animationduration / 1000) options:UIViewAnimationOptionTransitionNone animations:^{
                    CGAffineTransform transform = CGAffineTransformIdentity;
                    CGAffineTransform transformT = CGAffineTransformTranslate(transform, 40, -20);
                    CGAffineTransform transformS = CGAffineTransformScale(transform,1.0,1.0);
                    CGAffineTransform transformConcat = CGAffineTransformConcat(transformT, transformS);
                    imageView.transform = transformConcat;
                } completion:nil];
            }else if([imageName isEqualToString:@"light"]) {
                imageView.contentMode = UIViewContentModeScaleAspectFit;
                imageView.transform = CGAffineTransformTranslate(imageView.transform, 100, 0);
                imageView.transform = CGAffineTransformScale(imageView.transform,1.5,1.5);
				// 如果图像是灯光，则添加旋转动画
                [self setRotateAnimation:imageView];
            }
        }
		// 将图像视图添加到父视图中
...
// 设置旋转动画
- (void)setRotateAnimation:(UIImageView *)imageView
{
	// 设置图像视图的透明度
    imageView.alpha = 0.5;
	// 创建基本动画，将图像视图逆时针旋转30度
    ...
    imageView.layer.position = CGPointMake(viewSize.width, viewSize.height / 2);
	// 将动画添加到图像视图的图层中
...
- (void)hideViews
{
	...
	//20230417 :若存在则隐藏imageViews数组中对应视图
	if([IsOpenAnimationValue boolValue]){
...
- (void)destroyViews
{
    ...
    _activityView = nil;
    _curImageName = nil;
	//20230417 :若存在则销毁imageViews数组中对应视图
	if([IsOpenAnimationValue boolValue]){
		for (UIView *view in imageViews) {
